...
/// @custom:field lockedBond The amount of ETH bond that was locked by the
/// challenger.
/// @custom:field startBlock The block number at which the challenge was
/// initiated.
/// @custom:field resolvedBlock The block number at which the challenge was
/// resolved.
struct Challenge {
  address challenger;
  uint256 lockedBond;
...
/// @notice This contract enables data availability of a data commitment at a
/// given block number to be challenged.
///         To challenge a commitment, the challenger must first post a bond
/// (bondSize).
///         Challenging a commitment is only possible within a certain block
/// interval (challengeWindow) after the
///         commitment was made.
///         If the challenge is not resolved within a certain block interval
/// (resolveWindow), the challenge can be
///         expired.
///         If a challenge is expired, the challenger's bond is unlocked and the
/// challenged commitment is added to the
///         chain of expired challenges.
/// @custom:date June 18th, 2025.
contract DataAvailabilityChallenge is OwnableUpgradeable, ISemver {

  /// @notice Error for when the provided resolver refund percentage exceeds
  /// 100%.
  ...
  /// @notice Error for when attempting to challenge a commitment that already
  /// has a challenge.
  error ChallengeExists();

  /// @notice Error for when attempting to resolve a challenge that is not
  /// active.
  error ChallengeNotActive();

  /// @notice Error for when attempting to unlock a bond from a challenge that
  /// is not expired.
  error ChallengeNotExpired();

  /// @notice Error for when attempting to challenge a commitment that is not in
  /// the challenge window.
  error ChallengeWindowNotOpen();

  /// @notice Error for when the provided input data doesn't match the
  /// commitment.
  ...
  /// @notice Error for when the commitment length does not match the commitment
  /// type
  error InvalidCommitmentLength(
    uint8 commitmentType, uint256 expectedLength, uint256 actualLength
  );
  ...
  /// @notice An event that is emitted when the bond size required to initiate a
  /// challenge changes.
  event RequiredBondSizeChanged(uint256 challengeWindow);

  /// @notice An event that is emitted when the percentage of the resolving cost
  /// to be refunded to the resolver
  /// changes.
  ...
  /// @dev The value is estimated by measuring the cost of resolving with
  /// `bytes(0)`
  uint256 public constant fixedResolutionCost = 72925;

  /// @notice The variable cost of resolving a callenge per byte scaled by the
  /// variableResolutionCostPrecision.
  /// @dev upper limit; The value is estimated by measuring the cost of
  /// resolving with variable size data where each
  /// byte is non-zero.
  ...
  /// @notice The percentage of the resolving cost to be refunded to the
  /// resolver.
  ...
  /// @notice A mapping from challenged block numbers to challenged commitments
  /// to challenges.
  ...
  /// @param _challengeWindow The block interval during which a commitment can
  /// be challenged.
  ...
  /// @notice Sets the percentage of the resolving cost to be refunded to the
  /// resolver.
  /// @dev The function reverts if the provided percentage is above 100, since
  /// the refund logic
  /// assumes a value smaller or equal to 100%.
  /// @param _resolverRefundPercentage The percentage of the resolving cost to
  /// be refunded to the resolver.
  ...
  /// @notice Checks if the current block is within the challenge window for a
  /// given challenged block number.
  /// @param _challengedBlockNumber The block number at which the commitment was
  /// made.
  /// @return True if the current block is within the challenge window, false
  /// otherwise.
  ...
  /// @notice Checks if the current block is within the resolve window for a
  /// given challenge start block number.
  /// @param _challengeStartBlockNumber The block number at which the challenge
  /// was initiated.
  /// @return True if the current block is within the resolve window, false
  /// otherwise.
  ...
  /// @dev Unlike with a public `challenges` mapping, we can return a Challenge
  ...
  /// @notice Returns the status of a challenge for a given challenged block
  /// number and challenged commitment.
  /// @param _challengedBlockNumber The block number at which the commitment was
  /// made.
  /// @param _challengedCommitment The commitment that is being challenged.
    ...
    returns (ChallengeStatus)
  {
    ...
    // if the challenge's start block is not in the resolve window, it is
    // expired
  ...
  /// @param _commitment The commitment from which to extract the commitment
  /// type.
  ...
  /// @notice Validate that a given commitment has a known type and the expected
  /// length for this type.
  /// @dev The type of a commitment is stored in its first byte.
  ///      The function reverts with `UnknownCommitmentType` if the type is not
  /// known and
  ///      with `InvalidCommitmentLength` if the commitment has an unexpected
  /// length.
  ...
  /// @dev The block number parameter is necessary for the contract to verify
  /// the challenge window,
  ...
  ///      if the caller does not have a bond or if the challenge already
  /// exists.
  /// @param _challengedBlockNumber The block number at which the commitment was
  /// made.
  /// @param _challengedCommitment The commitment that is being challenged.
  ...
  /// @notice Resolve a challenge by providing the data corresponding to the
  /// challenged commitment.
  /// @dev The function computes a commitment from the provided resolveData and
  /// verifies that it matches the
  /// challenged commitment.
  ///      It reverts if the commitment type is unknown, if the data doesn't
    ...
    external
  {
    ...
    // require the provided input data to correspond to the challenged
    // commitment
    ...
    Challenge storage activeChallenge =
      challenges[_challengedBlockNumber][_challengedCommitment];
  ...
  /// @notice Distribute the bond of a resolved challenge among the resolver,
  /// challenger and address(0).
  ///         The challenger is refunded the bond amount exceeding the
  /// resolution cost.
  ///         The resolver is refunded a percentage of the resolution cost based
  /// on the `resolverRefundPercentage`
  ///         state variable.
  ///         The remaining bond is burned by sending it to address(0).
  /// @dev The resolution cost is approximated based on a fixed cost and
  /// variable cost depending on the size of the
  ///      pre-image.
  ///      The real resolution cost might vary, because calldata is priced
  /// differently for zero and non-zero bytes.
  ///      Computing the exact cost adds too much gas overhead to be worth the
  /// tradeoff.
    ...
    internal
  {
    ...
    uint256 resolutionCost = (
      fixedResolutionCost
        + _preImageLength * variableResolutionCost
          / variableResolutionCostPrecision
    ) * block.basefee;
    ...
    if (resolverRefund > lockedBond) resolverRefund = lockedBond;
    if (resolverRefund > 0) {
    ...
    if (lockedBond > 0) payable(address(0)).transfer(lockedBond);
    ...
    external
  {
    ...
    Challenge storage expiredChallenge =
      challenges[_challengedBlockNumber][_challengedCommitment];
    ...
    emit BalanceChanged(
      expiredChallenge.challenger, balances[expiredChallenge.challenger]
    );
  }
