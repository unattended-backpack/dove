...
/// @custom:proxied true
/// @title ETHLockbox
/// @notice Manages ETH liquidity locking and unlocking for authorized
/// OptimismPortals, enabling unified ETH liquidity
  ...
  ISemver
{
  ...
  /// @notice Thrown when the value to unlock is greater than the balance of the
  /// lockbox.
  error ETHLockbox_InsufficientBalance();

  /// @notice Thrown when attempting to unlock ETH from the lockbox through a
  /// withdrawal transaction.
  error ETHLockbox_NoWithdrawalTransactions();

  /// @notice Thrown when any authorized portal has a different
  /// SuperchainConfig.
  ...
  /// @notice Emitted when ETH is unlocked from the lockbox by an authorized
  /// portal.
  ...
  /// @notice Emitted when an ETH lockbox is authorized to migrate its liquidity
  /// to the current ETH lockbox.
  /// @param lockbox The address of the ETH lockbox that was authorized.
  event LockboxAuthorized(IETHLockbox indexed lockbox);

  /// @notice Emitted when ETH liquidity is migrated from the current ETH
  /// lockbox to another.
  ...
  /// @notice Emitted when ETH liquidity is received during an authorized
  /// lockbox migration.
  ...
  /// @param portal The address of an `OptimismPortal`.
  /// @param isAuthorized Whether or not the `portal` is authorized.
  mapping(IOptimismPortal portal => bool isAuthorized) public authorizedPortals;

  /// @notice Mapping of authorized lockboxes.
  mapping(IETHLockbox => mapping(address => bool)) public authorizedLockboxes;
  ...
  /// @dev Note: Multiple chains can share an ETHLockbox contract. In this case,
  /// all SystemConfig
  ///      contracts will point to the same pause identifier (the lockbox
  /// itself). Therefore, it
  ///      doesn't matter which SystemConfig is used here as long as it belongs
  /// to one of the
  ...
  ///         Called by an authorized portal when finalizing a withdrawal that
  /// requires ETH.
  ...
  /// @notice Authorizes an ETH lockbox to migrate its liquidity to the current
  /// ETH lockbox. We
  ///         allow this function to be called more than once for the same
  /// lockbox. A lockbox
  ...
  /// @dev    Must be called atomically with
  /// `OptimismPortal.migrateToSuperRoots()` in the same
  ///         transaction batch, or otherwise the OptimismPortal may not be able
  /// to unlock ETH
