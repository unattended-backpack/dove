//! Main collection entry point for source units

use crate::collector::comments::{
    find_trailing_comments, remove_comments_by_position, separate_internal_and_after_comments,
    take_last_comment_group_before,
};
use crate::collector::contracts::collect_contract_parts;
...
use crate::collector::types::{
    collect_enum_values, collect_error_parameters, collect_struct_fields,
    collect_type_comment_starts, collect_type_expression_comments,
};
...
/// Extract location from any SourceUnitPart
fn get_source_unit_part_location(part: &SourceUnitPart) -> (usize, usize) {
    match part {
        SourceUnitPart::PragmaDirective(p) => get_loc_range(&p.loc()),
        SourceUnitPart::ImportDirective(i) => match i {
            Import::Plain(_, loc) => get_loc_range(loc),
            Import::GlobalSymbol(_, _, loc) => get_loc_range(loc),
            Import::Rename(_, _, loc) => get_loc_range(loc),
        },
        SourceUnitPart::ContractDefinition(c) => get_loc_range(&c.loc()),
        SourceUnitPart::EnumDefinition(e) => get_loc_range(&e.loc()),
        SourceUnitPart::StructDefinition(s) => get_loc_range(&s.loc()),
        SourceUnitPart::EventDefinition(e) => get_loc_range(&e.loc()),
        SourceUnitPart::ErrorDefinition(e) => get_loc_range(&e.loc()),
        SourceUnitPart::FunctionDefinition(f) => get_loc_range(&f.loc()),
        SourceUnitPart::VariableDefinition(v) => get_loc_range(&v.loc()),
        SourceUnitPart::TypeDefinition(t) => get_loc_range(&t.loc()),
        SourceUnitPart::Annotation(a) => get_loc_range(&a.loc()),
        SourceUnitPart::Using(u) => get_loc_range(&u.loc()),
        SourceUnitPart::StraySemicolon(loc) => get_loc_range(loc),
    }
}

/// Main entry point for collecting source unit elements with comments
pub fn collect_source_unit(
    source_unit: &SourceUnit,
    comments: &[Comment],
    source: &str,
) -> CollectedElements {
    let mut collected = CollectedElements::new();
    let mut remaining_comments = comments.to_vec();

    // Process each source unit part in order
    for part in &source_unit.0 {
        let (start, end) = get_source_unit_part_location(part);
        ...
        match part {
            SourceUnitPart::PragmaDirective(pragma) => {
                collected.pragmas.push(CommentedElement {
                    element: pragma.clone(),
                    leading_comments,
                    trailing_comments,
                    type_comments: None,
                });
            ...
            SourceUnitPart::ImportDirective(import) => {
                collected.imports.push(CommentedElement {
                    element: Box::new(import.clone()),
                    leading_comments,
                    trailing_comments,
                    type_comments: None,
                });
            }
            SourceUnitPart::Using(using_directive) => {
            ...
            SourceUnitPart::VariableDefinition(var_def) => {
                ...
                let type_comments =
                    collect_type_expression_comments(&var_def.ty, &internal_comments, source);

                collected.variables.push(CommentedElement {
                    element: var_def.clone(),
                    leading_comments,
                    trailing_comments,
                    type_comments,
                });
            ...
            SourceUnitPart::FunctionDefinition(func_def) => {
                ...
                if !func_def.params.is_empty() {
                    let first_param_start = get_loc_start(&func_def.params[0].0);
                    let last_param_end = get_loc_end(&func_def.params[func_def.params.len() - 1].0);
                ...
                if !func_def.returns.is_empty() {
                    let first_return_start = get_loc_start(&func_def.returns[0].0);
                    let last_return_end =
                        get_loc_end(&func_def.returns[func_def.returns.len() - 1].0);
                    body_internal_comments.retain(|c| {
                        let c_start = get_loc_start(&c.loc());
                ...
                let (body_statements, body_standalone_comments) = if let Some(body) = &func_def.body
                {
            ...
            SourceUnitPart::ContractDefinition(contract) => {
                // Process contract with its internal comments
                let contract_contents =
                    collect_contract_parts(&contract.parts, &internal_comments, source);

                collected.contracts.push(CollectedContract {
                    definition: CommentedElement {
            ...
            SourceUnitPart::EventDefinition(_) => {
                // Events can only exist inside contracts, so this should never occur at top-level
            }
