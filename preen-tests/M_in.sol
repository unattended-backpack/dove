...
contract Test20 is
  ...
  */
  function name () public pure override(ITest20, ERC20) returns (string memory) {
    return "Test20";
  }
  ...
  */
  function _parseSalt (
    ...
    // The salt is unprotected with more randomness than the zero address alone and cross-chain use disallowed.
    } else if (bytes1(_salt[20]) == hex"01") {
      return (SenderBytes.Random, RedeployProtectionFlag.True);
    
    // The sale is unprotected with more randomness than the zero address alone and cross-chain use is allowed.
