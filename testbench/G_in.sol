...
contract EventLogger {
  ...
  function emitLog(bytes32[] calldata _topics, bytes calldata _data) external {
    ...
    assembly {
      ...
      calldatacopy(memDataOffset, _data.offset, dataSize) // args: to, from,
        // size
      // after the event-logging is done, the memory is not used, so no mem
      // pointer to update/restore.
  ...
  /// @notice Validates a cross chain message using the CrossL2Inbox predeploy.
  /// This emits an executing message.
